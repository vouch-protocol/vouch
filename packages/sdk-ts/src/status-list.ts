 ...
 * and suspension status per W3C VC-BITSTRING-STATUS-LIST
 ...
 * W3C BitstringStatusList §4.2: bit at index `i` is stored at byte `i >> 3`,
...
export class StatusList {
        ...
        if (length < DEFAULT_BITSTRING_LENGTH) {
            throw new StatusListError(
                `bitstring length must be at least ${DEFAULT_BITSTRING_LENGTH} ` +
                    `per W3C BitstringStatusList §4.2, got ${length}`
            );
        }
        if (length % 8 !== 0) {
            throw new StatusListError(
    ...
    static decode(
        ...
        if (length < DEFAULT_BITSTRING_LENGTH) {
            throw new StatusListError(
                `decoded bitstring length ${length} is below the W3C minimum ` +
