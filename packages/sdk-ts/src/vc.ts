 ...
 *
 * Mirrors `vouch/vc.py`. Builds a `VouchCredential` per W3C CG Report §5:
 * a W3C VC Data Model 2.0 credential carrying an agent's intent, optional
...
function validateIntent(intent: Intent): void {
    ...
    for (const required of ['action', 'target', 'resource'] as const) {
        ...
        if (v === undefined || v === null || v === '') {
            throw new Error(
                `intent.${required} is REQUIRED (W3C CG Report §5.4.1), ` +
