...
async def main():
    ...
    new_score = await engine.boost(did=agent.get_did(), amount=5, reason="Verified email identity")
    ...
    new_score = await engine.slash(
        did=agent.get_did(), amount=10, reason="Policy violation - unauthorized data access"
    )
