...
| [robotics_ai_act_evidence_pack.py](robotics_ai_act_evidence_pack.py) | Assembles robot credentials into a regulatory evidence pack covering all five conformance profiles (EU AI Act, ISO 10218, ISO/TS 15066, EU Machinery Regulation, UL 3300) and signs one attestation per profile. | `python robotics_ai_act_evidence_pack.py` |
| [robotics_vla_accountability_loop.py](robotics_vla_accountability_loop.py) | Runs a VLA control loop with provenance on load, a pre-actuation scope gate, and a tamper-evident black box. | `python robotics_vla_accountability_loop.py` |
| [robotics_openvla_gated_loop.py](robotics_openvla_gated_loop.py) | The same accountability loop driven by a real VLA model: loads OpenVLA (`openvla/openvla-7b`), attests the SHA-256 of the actual downloaded weight files, maps each predicted 7-DoF action into a `PhysicalAction`, and gates it before actuating. Needs the optional `openvla` extra and locally cached weights; skips cleanly without them. | `pip install 'vouch-protocol[openvla]'` then `python robotics_openvla_gated_loop.py` |
