...
def main() -> None:
    ...
    a_clock = capture_time_quality(a, clock=SimulatedClock("gnss", since_discipline_s=8.0, uncertainty_s=0.4))
    a_integrity = capture_integrity_risk(a, monitor=SimulatedIntegrityMonitor(0.12, {"doseRad": 180, "seu": 0}))
    ...
    with network_disabled():
        ...
        ok_fresh, _ = verify_freshness_token(
            a_freshness, anchors[relay.get_did()],
            verifier_epoch=epoch.current_epoch(), tier=CONSEQUENCE_CRITICAL,
        )
        ...
        att = capture_presence_attestation(
            b, peer_did=a.get_did(), nonce="maneuver-42",
            claimed_position=A_POSITION, range_sensor=b_radio, tolerance_m=1.0,
        )
        ok_present, _ = verify_presence_live(att, b_kp.public_key_jwk, nav=b_nav, expected_nonce="maneuver-42")
        ...
        att_spoof = capture_presence_attestation(
            b, peer_did=a.get_did(), nonce="x",
            claimed_position=A_POSITION, range_sensor=b_radio_spoof, tolerance_m=1.0,
        )
        ...
        observers = [
            (b, SimulatedNavigation([0, 0, 0], [0, 0, 0]), SimulatedRangeSensor({a.get_did(): 100.0})),
            (obs2, SimulatedNavigation([200, 0, 0], [0, 0, 0]), SimulatedRangeSensor({a.get_did(): 100.0})),
            (obs3, SimulatedNavigation([0, 200, 0], [0, 0, 0]), SimulatedRangeSensor({a.get_did(): 100.0})),
        ]
        subjects = []
        for signer_obs, nav, radio in observers:
            o = capture_range_observation(
                signer_obs, target_did=a.get_did(), nav=nav, range_sensor=radio,
                nonce="tri-42", epoch_source=epoch,
            )
        ...
        print(f"  [location] {len(subjects)} signed observations, position confirmed (>=2): {confirmed}")
        ...
        print(f"  [time] A's attested clock uncertainty {tq['uncertaintyS']}s ok for critical: {ok_time}")

        # 2e. Kinematics: is A where it now claims, reachably, from 10 s ago?
        prior = [A_POSITION[0], A_POSITION[1] - A_VELOCITY[1] * 10, A_POSITION[2]]  # 50 m south, 10 s ago
        a_nav_now = SimulatedNavigation(A_POSITION, A_VELOCITY)
        reachable = check_kinematics_live(
            prior_position=prior, nav=a_nav_now, elapsed_seconds=10.0,
        ...
        print(f"  [integrity] A's cumulative risk {isub['cumulativeRisk']} -> authority level '{level}'")

        # Composite decision.
        trust_for_maneuver = ok_fresh and ok_present and confirmed and ok_time and reachable and level == "full"
