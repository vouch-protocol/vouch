...
app = FastAPI(
    title="Vouch Protected API",
    description="Example API protected with Vouch-Token verification"
)
...
async def protected_resource(
    vouch_token: Optional[str] = Header(None, alias="Vouch-Token")
):
    """
    ...
    if not vouch_token:
        raise HTTPException(
            status_code=400,
            detail="Missing Vouch-Token header"
        )
    ...
    if TRUSTED_PUBLIC_KEY:
        ...
        is_valid, passport = Verifier.verify(
            vouch_token,
            public_key_jwk=TRUSTED_PUBLIC_KEY
        )
    else:
    ...
    if not is_valid or not passport:
        raise HTTPException(
            status_code=401,
            detail="Invalid or expired Vouch-Token"
        )
...
async def agent_connect(
    ...
    is_valid, passport = Verifier.verify(
        vouch_token,
        public_key_jwk=TRUSTED_PUBLIC_KEY
    ) if TRUSTED_PUBLIC_KEY else verifier.check_vouch(vouch_token)
    
    if not is_valid or not passport:
        raise HTTPException(
            status_code=401,
            detail="Agent Verification Failed"
        )
    
    return {
        "status": "Connected",
        "agent": passport.sub,
        "token_id": passport.jti
    }
