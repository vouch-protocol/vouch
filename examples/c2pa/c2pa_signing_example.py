        ...
        "assertions": [
            {
                "label": "c2pa.actions",
                "data": {
                    "actions": [{"action": "c2pa.created", "softwareAgent": "Vouch Protocol v1.0"}]
                },
            },
            {
                ...
                "data": {
                    ...
                    "@type": "CreativeWork",
                    "author": [{"@type": "Organization", "name": "Vouch Protocol"}],
                },
            },
        ],
    }
...
def sign_image_with_c2pa(
    ...
    signer_info = c2pa.C2paSignerInfo(
        alg="PS256", sign_cert=sign_cert, private_key=private_key, ta_url=""
    )
    ...
    with open(input_path, "rb") as source:
        with open(output_path, "wb") as dest:
            result = builder.sign(signer_info, SUPPORTED_FORMATS[ext], source, dest)
...
def main():
    ...
    if args.verify:
        ...
        if not args.key or not args.cert:
            ...
            print(
                "Example: python c2pa_signing_example.py photo.jpg --key leaf.key --cert chain.pem"
            )
            sys.exit(1)

        output_path = (
            Path(args.output) if args.output else input_path.with_stem(f"{input_path.stem}_signed")
        )

        print(f"Signing: {input_path} -> {output_path}")
        try:
            result = sign_image_with_c2pa(
                input_path, output_path, Path(args.key), Path(args.cert), args.title
            )
            print(json.dumps(result, indent=2))
