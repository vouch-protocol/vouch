...
if not os.path.exists("vouch"): os.makedirs("vouch")
with open("vouch/cli.py", "w") as f: f.write(cli_content)
print("Created vouch/cli.py")

# 2. UPDATE CONFIG & BUMP VERSION 1.0.8
try:
    with open("pyproject.toml", "r") as f: txt = f.read()
    
    if "[project.scripts]" not in txt:
        txt += '\n[project.scripts]\nvouch = "vouch.cli:main"\n'
    
    txt = re.sub(r'version = "[0-9]+\.[0-9]+\.[0-9]+"', 'version = "1.0.8"', txt)
    
    with open("pyproject.toml", "w") as f: f.write(txt)
    print("Bumped version to 1.0.8")
except FileNotFoundError: pass

# 3. UPDATE DOCS
doc = "\n### CLI\n`vouch init --domain x.com`\n`vouch sign 'msg'`\n`vouch verify 'tok'`\n"
try:
    with open("README.md", "r") as f: r = f.read()
    if "### CLI" not in r:
        with open("README.md", "w") as f: f.write(r + doc)
except FileNotFoundError: pass
