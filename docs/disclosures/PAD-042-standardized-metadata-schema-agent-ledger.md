...
a W3C Data Integrity proof (`eddsa-jcs-2022` or
...
`credentialSubject.execution` field of a W3C Verifiable Credential, or
equivalently the JWS payload `vouch.execution` field for the legacy
v0.x form.

| Field | Type | Description |
|---|---|---|
| `agent_did` | DID URI | The W3C Decentralized Identifier of the executing machine (e.g., `did:web:agent.example.com`). Resolves to a DID Document containing the verification method that signed this execution record. |
...
W3C Data Integrity proof yields a record that any independent party
...
**Embodiment 1: W3C VC + Data Integrity ledger entry.** The schema
...
verify both the W3C Data Integrity proof on each entry and the Merkle
...
Combined with the W3C Data Integrity proof attached to the schema and
