...
| Post-quantum signing (optional) | Ed25519 + ML-DSA-44 (FIPS 204) | Quantum-safe migration path; both proofs must validate |
| Cryptosuite (default) | `eddsa-jcs-2022` (Data Integrity) | standards-aligned; no JOSE/JWS dependency in v1.0 |
| Cryptosuite (post-quantum) | `mldsa44-jcs-2024`, carried alongside `eddsa-jcs-2022` as a proof set | Defined per Specification §13.2 |
...
The post-quantum profile addresses two specific threats:

| Threat | How the post-quantum profile mitigates |
|---|---|
| **Harvest-Now, Decrypt-Later (signatures)** | Vouch credentials are short-lived (default 5 min) so encryption-style harvest attacks are not the primary concern. The post-quantum profile's relevance is *retroactive forgery*: if a quantum adversary breaks Ed25519 in 2030, they could forge a signature dated 2026. The proof set binds the credential under **both** Ed25519 and ML-DSA-44, and both proofs must verify, so a compromise of Ed25519 alone cannot produce an acceptable credential. |
| **Algorithm transition risk** | NIST CNSA 2.0 and U.S. NSM-10 mandate quantum-resistant cryptography on phased timelines (CNSA 2.0 phases starting 2027). Adopting the post-quantum profile now ensures continued validity through and beyond the migration cutoff. |

The default `eddsa-jcs-2022` cryptosuite provides the same classical
security level as v0.x. Implementations MAY support the post-quantum profile,
and it is expected to become RECOMMENDED for regulated sectors.
