...
per W3C CG Report §11.6. A heartbeat request is validated by N
...
class QuorumPolicy:
    ...
    initial_trust_aggregator: Callable[[List[float]], float] = field(
        default_factory=lambda: min
    )
    decay_lambda_aggregator: Callable[[List[float]], float] = field(
        default_factory=lambda: max
    )
    max_ttl_aggregator: Callable[[List[int]], int] = field(
        default_factory=lambda: min
    )
...
class HeartbeatQuorum:
    ...
    def validate(self, request: Dict[str, Any]) -> QuorumResult:
        ...
        if votes_for < self.threshold:
            return QuorumResult(
                ...
                votes_for=votes_for,
                approving_dids=[
                    qv.validator.validator_did for qv in approving
                ],
        ...
        try:
            ...
            raise QuorumError(
                f"request shape changed between validator runs: {exc}"
            ) from exc
    ...
    def _build_aggregate_voucher(
        ...
        return build_session_voucher(
            ...
            valid_seconds=int(
                self.policy.voucher_valid_seconds_aggregator(validity_windows)
            ),
        )
