...
class Capabilities:
    ...
    def from_dict(cls, data: Dict[str, str]) -> "Capabilities":
        ...
        return cls(
            ...
            custom={k: v for k, v in data.items() if k not in ("filesystem", "network", "shell")},
        )
...
class PermissionManager:
    ...
    def save_config(self) -> None:
        ...
        config = {
            ...
            "capabilities": {did: caps.to_dict() for did, caps in self._capabilities.items()},
        }
        ...
        for resource, required in requirements.items():
            if resource == "filesystem":
                if not self._check_level(
                    caps.filesystem.value, required, ["none", "read", "write", "full"]
                ):
                    return (
                        False,
                        f"Insufficient filesystem permission: requires {required}, has {caps.filesystem.value}",
                    )

            elif resource == "network":
                if not self._check_level(
                    caps.network.value, required, ["none", "internal", "outbound", "full"]
                ):
                    return (
                        False,
                        f"Insufficient network permission: requires {required}, has {caps.network.value}",
                    )

            elif resource == "shell":
                if not self._check_level(caps.shell.value, required, ["none", "sandboxed", "full"]):
                    return (
                        False,
                        f"Insufficient shell permission: requires {required}, has {caps.shell.value}",
                    )
