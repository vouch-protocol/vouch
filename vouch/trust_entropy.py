...
W3C CG Report §11.5. A SessionVoucher carries `initialTrust` and
...
# Threshold band conventions described in CG Report §11.5. Implementers MAY
...
def compute_trust_at(
    ...
    try:
        ...
        raise TrustEntropyError(
            f"credentialSubject.initialTrust must be a number: {exc}"
        ) from exc
    ...
    try:
        ...
        raise TrustEntropyError(
            f"credentialSubject.decayLambda must be a number: {exc}"
        ) from exc

    if initial_trust < 0:
        raise TrustEntropyError(
            f"initialTrust must be non-negative, got {initial_trust}"
        )
    if decay_lambda < 0:
        raise TrustEntropyError(
            f"decayLambda must be non-negative, got {decay_lambda}"
        )
...
def _parse_iso_utc(iso_str: str) -> datetime:
    """Parse the W3C-VC ISO timestamp format used in Vouch credentials."""
