...
class MemoryReputationStore(ReputationStoreInterface):
    ...
    def __init__(self):
        ...
        self._stats: Dict[str, Dict] = defaultdict(lambda: {
            "successes": 0, "failures": 0, "last_action_at": None
        })
...
class RedisReputationStore(ReputationStoreInterface):
    ...
    async def get_events(self, did: str, limit: int = 100) -> List[ReputationEvent]:
        ...
        try:
            raw_events = await self._redis.lrange(
                self._events_key(did), 0, limit - 1
            )
            events = []
    ...
    async def get_stats(self, did: str) -> Dict:
        try:
            ...
            return {"total_actions": 0, "successes": 0, "failures": 0, 
                    "success_rate": 0.0, "last_action_at": None}
...
class KafkaReputationConsumer:
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            raise ImportError(
                "Kafka consumer requires: pip install aiokafka redis"
            )
...
class ReputationEngine:
    ...
    def __init__(
        self,
        store: Optional[ReputationStoreInterface] = None,
    ...
    async def _apply_delta(
        ...
        new_score = max(
            ...
            min(self._config["max_score"], current + delta)
        )
