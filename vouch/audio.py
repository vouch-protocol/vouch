...
class AudioSealWatermarker(AudioWatermarker):
    ...
    def __init__(self, model_name: str = "audioseal_wm_16bits"):
        ...
        if not AUDIOSEAL_AVAILABLE:
            raise ImportError("AudioSeal not available. Install with: pip install audioseal torch")
...
class SpreadSpectrumWatermarker(AudioWatermarker):
    ...
    def detect(
        ...
        if len(audio_data) < 1000:
            return WatermarkVerificationResult(
                detected=False, confidence=0.0, error="Audio too short for detection"
            )
...
class AudioSigner:
    ...
    def sign_c2pa(
        ...
        if not source.exists():
            return SignedAudioResult(
                success=False, source_path=str(source), error=f"Source file not found: {source}"
            )
        ...
        if ext not in SUPPORTED_AUDIO_FORMATS:
            return SignedAudioResult(
                success=False, source_path=str(source), error=f"Unsupported audio format: {ext}"
            )
        ...
        try:
            ...
            return SignedAudioResult(success=False, source_path=str(source), error=str(e))
    ...
    def _load_audio(self, path: Path) -> tuple["np.ndarray", int]:
        ...
        raise ImportError("No audio library available. Install one of: scipy, soundfile, librosa")
    ...
    def _save_audio(self, data: "np.ndarray", sample_rate: int, path: Path):
        ...
        raise ImportError("No audio library available. Install one of: scipy, soundfile")
