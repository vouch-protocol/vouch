...
Builds a `VouchCredential` per W3C CG Report §5: a W3C VC Data Model 2.0
...
def build_session_voucher(
    ...
    *,
    ...
    """
    Construct an unsigned SessionVoucher credential (W3C CG Report §11.4).
...
def _validate_intent(intent: Dict[str, Any]) -> None:
    ...
    for required in ("action", "target", "resource"):
        if required not in intent or intent[required] in (None, ""):
            raise ValueError(
                f"intent.{required} is REQUIRED (W3C CG Report §5.4.1) — "
