"""
HTTP fetcher for W3C BitstringStatusListCredential (W3C CG Report §11.2).
    ...
    def get(self, url: str, *, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        if not url.startswith("https://"):
            raise StatusListFetchError(
                f"status list URL must be https://, got {url!r}"
            )
        ...
        if len(body) > self.max_response_bytes:
            raise StatusListFetchError(
                f"response too large ({len(body)} bytes) for {url}"
            )
