...
referenced in W3C CG Report §11.3. An agent records signals as it runs
...
class BehavioralCollector:
    ...
    def record_api_call(
        ...
        *,
        ...
        if drift is not None and not 0.0 <= drift <= 1.0:
            raise BehavioralAttestationError(
                f"drift must be in [0, 1], got {drift}"
            )
    ...
    def digest(self) -> Dict[str, Any]:
        ...
        heartbeat request (CG Report §11.3).
        """
        with self._lock:
            return {
...
def validate_behavioral_digest(digest: Dict[str, Any]) -> None:
    ...
    for field_name in ("apiCalls", "tokensConsumed"):
        ...
        if not isinstance(digest[field_name], int) or digest[field_name] < 0:
            raise BehavioralAttestationError(
                f"digest.{field_name} must be a non-negative integer"
            )
    ...
    for r in resources:
        if not isinstance(r, str):
            raise BehavioralAttestationError(
                "digest.resourcesAccessed entries must be strings"
            )
    ...
    if not isinstance(drift, (int, float)):
        raise BehavioralAttestationError(
            "digest.intentDriftScore must be a number"
        )
    if not 0.0 <= float(drift) <= 1.0:
        raise BehavioralAttestationError(
            f"digest.intentDriftScore must be in [0, 1], got {drift}"
        )
