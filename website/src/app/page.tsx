...
const FEATURES: Array<{ num: string; title: string; body: string; spec: string; href?: string }> = [
  ...
  },
  {
    ...
    body: 'Freshness is elapsed time, authority state, and consequence together. An authority publishes a signed state credential carrying a counter that only goes up, and any transition bumps it. A verifier that has seen the newer counter refuses a session voucher minted under the older one for a high-consequence action, even while its time-decay trust still passes. The comparison is local, so a disconnected verifier keeps working; the highest tier adds a live M-of-N co-sign read at the moment of the action.',
    spec: 'Authority Freshness',
  },
  {
    num: 'vii.',
