"""
Unit tests for Canary Commitments (Specification §11.3, §11.7).
"""
...
class TestVerifyReveal:
    def test_correct_reveal_verifies(self):
        ...
        commitment = compute_commitment(secret)
        encoded_secret = (
            MULTIBASE_BASE64URL_PREFIX + base64.urlsafe_b64encode(secret).rstrip(b"=").decode()
        )
    ...
    def test_wrong_reveal_fails(self):
        ...
        encoded = (
            MULTIBASE_BASE64URL_PREFIX
            + base64.urlsafe_b64encode(wrong_secret).rstrip(b"=").decode()
        )
