"""
Unit tests for the BitstringStatusList implementation (Specification §11.2).
"""
...
class TestVerifyStatus:
    ...
    def test_purpose_mismatch_raises(self):
        sl = StatusList(status_list_id=STATUS_URL, status_purpose=STATUS_PURPOSE_SUSPENSION)
        vc = build_status_list_credential(
            issuer_did="did:web:issuer.example",
            status_list=sl,
        )
...
class TestStatusListFetcher:
    ...
    def test_verify_status_with_fetched_credential(self):
        ...
        assert (
            verify_status(credential_status=revoked_entry, status_list_credential=fetched) is True
        )
        assert (
            verify_status(credential_status=active_entry, status_list_credential=fetched) is False
        )
