...
from vouch.robotics import (
    # freshness (107, 119)
    build_freshness_token, verify_freshness_token, decay_weight, decay_permits,
    # dtn_revocation (112, 120)
    build_conditional_revocation, verify_conditional_revocation, conditional_revocation_active,
    build_validity_root, build_validity_witness, verify_validity_witness,
    # localization (113, 114, 121)
    build_range_observation, verify_range_observation, count_consistent, location_confirmed,
    kinematically_reachable, within_beam, build_beam_presence, verify_beam_presence,
    # quorum_trust (110, 111, 116)
    build_distress_attestation, verify_distress_attestation, is_quarantined,
    build_trust_state_update, verify_trust_state_update, accept_trust_state_update,
    build_key_continuity_predelegation, build_continuity_approval, verify_key_continuity,
    # edge_trust (115, 117, 118)
    build_time_quality_attestation, time_quality_permits,
    build_autonomy_schedule, verify_autonomy_schedule, select_envelope, autonomy_permits,
    build_integrity_risk_attestation, integrity_authority_level,
    # perception_consensus (122, 123)
    build_perception_claim, verify_perception_claim, cross_check_perception,
    build_interaction_attestation, node_standing,
    # bundle (124)
    bind_credential_to_bundle, verify_bundle_trust, build_custody_transfer, custody_chain_ok,
)
...
def test_conditional_revocation_fires_without_renewal():
    ...
    cr = build_conditional_revocation(auth, target_credential_id="cred-1", subject_did="did:web:node", deadline_epoch=100)
    ...
    assert not conditional_revocation_active(sub, current_epoch=101, last_renewal_epoch=100)  # renewed at deadline
...
def test_validity_witness_roundtrip_and_negative():
    ...
    assert verify_validity_witness(witness=w, signed_root_credential=root, authority_public_key=kp.public_key_jwk)
    # a witness against a different (revoked) set/root must fail
    root2 = build_validity_root(auth, valid_ids=["cred-a", "cred-b"], epoch=8)
    assert not verify_validity_witness(witness=w, signed_root_credential=root2, authority_public_key=kp.public_key_jwk)
...
def test_range_observation_signs_and_verifies():
    ...
    o = build_range_observation(obs, target_did="did:web:t", observer_position=[1,2,3], measured_range_m=10.0, nonce="n", epoch=1)
...
def test_kinematic_reachable_surface_and_orbital():
    ...
    assert kinematically_reachable(prior_position=[0,0,0], claimed_position=[40,0,0], elapsed_seconds=10, envelope={"maxSpeedMps":5})
    assert not kinematically_reachable(prior_position=[0,0,0], claimed_position=[80,0,0], elapsed_seconds=10, envelope={"maxSpeedMps":5})
    # orbital: |v0|=100 + dv 10 over 1 s => reach 110
    assert kinematically_reachable(prior_position=[0,0,0], claimed_position=[105,0,0], elapsed_seconds=1,
                                   envelope={"maxDeltaVMps":10}, prior_velocity=[100,0,0])
    assert not kinematically_reachable(prior_position=[0,0,0], claimed_position=[200,0,0], elapsed_seconds=1,
                                       envelope={"maxDeltaVMps":10}, prior_velocity=[100,0,0])
...
def test_within_beam_and_attestation():
    ...
    att = build_beam_presence(s, peer_did="did:web:peer", nonce="n", pointing=[1,0,0], beamwidth_rad=math.radians(10))
    ok, _ = verify_beam_presence(att, kp.public_key_jwk, peer_direction=[1,0.02,0], expected_nonce="n")
...
def test_distress_signs_and_verifies():
    ...
    d = build_distress_attestation(m, target_did="did:web:bad", reason="out_of_envelope", evidence_ref="frame:abc", epoch=5)
...
def test_accept_update_needs_distinct_domains_and_no_rollback():
    ...
    subs = [
        ...
        {"scope": "revocations", "change": change, "epoch": 10, "failureDomain": "orbit-A"},  # same domain
    ]
...
def test_key_continuity_threshold_approvals():
    ...
    pre = build_key_continuity_predelegation(auth, mission_credential_id="mission-1", member_dids=members, threshold=2)
    ...
    assert verify_key_continuity(predelegation_subject=pre_subject, reissuance_id="reissue-1",
                                 supersedes="mission-1", approval_subjects=approvals)
    assert not verify_key_continuity(predelegation_subject=pre_subject, reissuance_id="reissue-1",
                                     supersedes="mission-1", approval_subjects=approvals[:1])

# ---- PAD-115 time quality ------------------------------------------------- #

def test_time_quality_gate():
    s, kp = signer("node.example")
    good = build_time_quality_attestation(s, source_class="gnss", since_discipline_s=5, uncertainty_s=0.5)
    ...
    poor = build_time_quality_attestation(s, source_class="rc-oscillator", since_discipline_s=1e6, uncertainty_s=120.0)
...
def test_node_standing_recent_and_distinct():
    subs = [
        ...
        {"id": "did:web:n", "attestor": "did:web:p1", "outcome": "ok", "epoch": 80},  # older dup for p1
    ]
...
def test_bundle_trust_and_custody_chain():
    ...
    bc = bind_credential_to_bundle(orig, bundle_id="b-1", payload_hash="sha256:abc", intent={"action": "deliver"})
    ...
    transfers = [
        ...
        {"id": "b-1", "custodian": "did:web:relay2", "previousCustodian": "did:web:relay1", "epoch": 2},
    ]
    assert custody_chain_ok(transfers, bundle_id="b-1", originator=origin_did)
    broken = [transfers[0], {"id": "b-1", "custodian": "did:web:relay2", "previousCustodian": "did:web:GHOST", "epoch": 2}]
