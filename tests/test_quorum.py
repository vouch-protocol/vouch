...
class TestQuorumConstruction:
    ...
    def test_negative_weight_rejected(self):
        with pytest.raises(QuorumError):
            QuorumValidator(validator=_validator("did:web:v1.example.com"), weight=-1.0)


class TestQuorumValidation:
    def _build_2_of_3(self):
        v1 = QuorumValidator(validator=_validator("did:web:v1.example.com"), role=ROLE_POLICY)
        v2 = QuorumValidator(validator=_validator("did:web:v2.example.com"), role=ROLE_BEHAVIORAL)
        v3 = QuorumValidator(validator=_validator("did:web:v3.example.com"), role=ROLE_BUDGET)
...
class TestWeightedQuorum:
    def test_weighted_validator_counts_more(self):
        v1 = QuorumValidator(validator=_validator("did:web:senior.example.com"), weight=2.0)
        v2 = QuorumValidator(validator=_validator("did:web:junior.example.com"), weight=1.0)
